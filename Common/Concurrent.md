...
- 忽略（Ingnre）：对这个信号不做任何处理。但是 SIGKILL 和 SIGSTOP 这个两个信号，进程是不能忽略的，因为它们的主要作用是为 Linux kernel 和超级用户提供删除任意进程的特权。
- 捕获（Catch）：让用户进程可以注册自己针对这个信号的 handler。SIGKILL 和 SIGSTOP 这个两个信号同样不能由用户自己处理，只能执行系统的缺省行为。
- 缺省（Default）：一般会采用退出（terminate）、暂停（stop）、忽略（ignore）的某一种
...
- **SIGTERM（15）和 SIGKILL（9）两个信号**

`kill` 命令默认发出的就是 SIGTERM 信号。该信号可以被捕获，从而执行用户自定义的 handler，可以处理进程的 graceful shutdown 问题；

`kill -9 pid` 命令就是发送 SIGKILL 信号给指定 pid 的进程。SIGKILL 信号不能被忽略，也不能被捕获，进程一旦收到该信号就要退出。	
