...
容器是一个**视图隔离、资源可限制、独立文件系统**的单进程模型，**并不是指容器里只能运行“一个”进程，而是指容器没有管理多个进程的能力**。
...
补充：CFS （Completely Fair Scheduler，即完全公平调度器）是 Linux 地普通调度算法，cpu.cfs_period_us 是 CFS 算法的一个调度周期，pu.cfs_quota_us 表示 CFS 算法中一个调度周期里这个控制组被允许的运行时间。
