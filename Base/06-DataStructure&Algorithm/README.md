...
- 在图灵机（Turing Machine, TM）和随机存储机（Random Access Machine, RAM）等计算模型中，指令语句均可分解为若干次基本操作；而在大多数实际的计算环境中，每一次这类基本操作都可在常数时间内完成；
- 将时间复杂度理解为算法中各条指令的执行时间之和。更进一步将**$T(n)$定义为算法所执行基本操作的总次数**。也就是说，$T(n)$决定于组成算法的所有语句各自的执行次数，以及其中所含基本操作的数目。
...
涉及索引时如何定义清楚需求？罪域系统设计需求，可分为功能性需求、非功能性需求：
