...
| and                | 注意：SQL中 and 优先级要比 or 高                             |
| or                 |                                                              |
| not                | 取非，主要用在 is 或 in 中                                   |
| in                 | 包含在几个选项之中，相当于多个 or（not in不在这个范围内）    |
| like               | 模糊查询，支持 %或下划线匹配：`%`匹配任意个字符；一个下划线只匹配一个字符 |

案例：找出工资大于1800的员工，要求部门编号为20或30。 
...
- 案例：筛选出 title 列中值为 sun 或 color 的记录（数据）
...
- 按多列排序（先按col1排序，重复的时候按col2排序...）：

  ```sql
  select * from table_name [where 子句] order by col1 [asc/desc]，col2 [asc/desc]...  
  -- 注意：不加asc或desc时，默认asc升序；where子句和order by不能调换顺序 
  ```
...
应用场景：要查询的结果来自于多个表，且多个表没有直接的连接关系，但查询的信息一致（如查询中外的男性，都有id、name、gender）时 。
...
- FROM后面的子查询：将子查询结果充当一张表，要求**必须起别名** 
