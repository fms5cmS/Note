...
第一个用户态的进程即 init 进程，其 pid 为 1，该进程会创建并管理 Linux 系统中其他所有的进程。该进程永远不能被 SIGKILL 杀死，容器中 pid = 1 的进程有时（SIGTERM 信号被捕获后的默认 handler 被修改）可以被 SIGTERM 信号杀死。

一台 Linux 机器上的进程总数是有限的，可以在 /proc/sys/kernel/pid_max 参数中查看总数（内核初始化系统时根据 CPU 总数设置），一旦超过总数就无法创建出新的进程了。
