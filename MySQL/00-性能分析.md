性能下降导致SQL慢，其结果：执行时间长 或 等待时间长。主要原因：
...
- `type`：访问类型排列。下面**从最差到最好**列出了常见的取值：
  ...
  - `index`：即 Full Index Scan。`index `与 `ALL`的区别为index类型只遍历索引树。这通常比`ALL`快，因为索引文件通常比数据文件小。（也就是说虽然`ALL`和`index`都是读全表，但`index`是从索引中读取的，而`ALL`是从硬盘中读的）；
