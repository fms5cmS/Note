...
`a := make([]*Person, 0, 0)` 和 `a := []*Person{}` 两种写法一定会分配内存，而 `var a []*Person` 的写法如果后续没有用到 a，那就不需要分配内存

切片字面量的初始化，会**以数组的形式存储在静态区**，在使用 make 函数初始化时，如果初始化了一个大于 64KB 的切片，那么这个切片会逃逸到堆中，在运行时调用 makeslice 函数创建切片，小于 64KB 的切片直接在栈中初始化。
...
func TestGet(t *testing.T) {
    x := make([]int,2, 10)
    _ = x[6:10] // 注意：这里是正常的，切片的这个索引范围最大值为切片的容量
    _ = x[6:]  // 报错，截取的索引范围为 [6, len(arr))，报错是因为 len(arr)==2，小于起始下标 6
    _ = x[2:]
}
```
...
- 如果新申请的容量大于旧容量的两倍，则最终容量是新申请的容量；
- 否则：
  - 如果旧切片的长度小于 1024，则最终容量是旧容量的两倍
  - 否则如果旧切片的长度大于等于 1024，则最终容量从旧容量开始循环增加原来的 1/4，直到最终容量大于等于新申请的容量为止
  - 如果最终容量计算值过大导致溢出（超出 int 的最大范围），则最终容量就是新申请容量。
...
**大量使用 init 函数，如果之间存在依赖，初始化顺序不对（如对文件 `go fmt` 后），可能会导致程序启动不了！！**
...
type Node struct {
    Value       int
    Left, Right *Node // 如果要内嵌自身，必须使用指针类型，使用非指针类型的话编译器不知道该结构体的大小
}
