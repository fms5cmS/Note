...
`sync.Pool` 类型可以被称为临时对象池，其值可以被用来存储临时的对象。

这里临时对象的意思是：不需要持久使用的某一类值，其创建和销毁可以在任何时候发生，且完全不影响程序功能。而且他们也应该是无需被区分的，其中任何一个值可以代替另一个。

所以可以将临时对象池当作针对某种数据的缓存来使用！
...
- `Get() interface{}` 用于从当前池中随机取出临时对象
  ...
  - New 字段的实际值需要在初始化 Pool 的时候就给顶，否则调用 Get 方法可能会得到 nil
